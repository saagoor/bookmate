<x-admin-layout title="Books" :show-search="true">
        
    <x-slot name="actions">
        <x-link-button :href="route('admin.books.create')"><x-heroicon-o-plus class="h-5 -mt-0.5 mr-1" />Add New</x-link-button>
    </x-slot>
            ...
            @forelse ($books as $book)
                <tr>
                    ...
                    <td class="px-2 py-1 border">
                        <img class="h-10 rounded" data-src="{{ $book->cover_url }}">
                    </td>
                    <td class="px-2 py-1 border">{{ $book->name }}</td>
                    <td class="px-2 py-1 border"><a href="{{ request()->fullUrlWithQuery(['category' => $book->category]) }}">{{ $book->category }}</a></td>
                    <td class="px-2 py-1 border">
                        @foreach ($book->writers as $writer)
                            <p><a href="{{ request()->fullUrlWithQuery(['writer' => $writer->id]) }}">{{ $writer->name }}</a></p>
                        @endforeach
                    </td>
                    <td class="px-2 py-1 border">
                        @forelse ($book->translators as $translator)
                            <p><a href="{{ request()->fullUrlWithQuery(['translator' => $translator->id]) }}">{{ $translator->name }}</a></p>
                        @empty
                            <p>N/A</p>
                        @endforelse
                    </td>
