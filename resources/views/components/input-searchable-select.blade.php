...
<div {{ $attributes->only('class') }}>
    ...
    @if ($slot)
        <x-label :for="$name" class="mb-1">{{ $slot }}</x-label>
    @endif

    <div x-data="searchableSelector({{ $value }})"
         @click.away="close()"
         class="relative">
        <div class="w-full">
            <div class="relative">
                ...
                <x-input name=""
                         ...
                         placeholder="{{ $placeholder ?? 'Select ' . $slot }}"
                         readonly
                         @focus="open()"
                         class="w-full"/>
                <button type="button"
    ...
    @push('head')
        ...
        <script>
            document.addEventListener('alpine:init', () => {
                Alpine.data('searchableSelector', (oldValue) => ({
                    filter: '',
                    show: false,
                    ...
                    classOption(id, index) {
                        ...
                        return {
                            ...
                            'bg-primary-200': isSelected || isFocused,
                        };
                    },
                    filterOptions() {
                    ...
                    init() {
                        if (oldValue) {
                            this.selected = this.options.find(item => item.id === oldValue);
                        }
                    }
                }))
            })
