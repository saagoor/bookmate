@foreach($steps as $index => $step)
                ...
                <div x-data="{show: {{ $errors->any() ? 'true' : 'false' }}}" class="font-sans">
                    ...
                    <x-modal-form :title="$step['content']" :action="route('exchanges.pickup', $exchange)">
                        <x-input-text
                                ...
                                class="mb-3"
                                placeholder="Bashundhara, Panthapath">Location
                        </x-input-text>
                        <x-input-text
                                name="pickup_time"
    ...
    @if (!$exchange->is_ebook)
        ...
        @if($index == 2 && $steps[$index]['condition'])
            <p class="font-sans text-sm font-normal text-gray-600 text-right mb-2">
                {{ $exchange->pickup_location }}
                - {{ $exchange->pickup_time->format('d M Y - g:i A') }}
            </p>
        @endif
        ...
        @if($index == 3 && $steps[$index]['condition'])
            ...
            <p class="font-sans text-lg font-semibold text-green-600 leading-tight mb-2">
        ...
        @if ($index == 2 && $steps[$index]['condition'])
            <div class="my-2">
                @if ($exchange->user_id == auth()->user()->id)
                    <x-link-button target="_blank" :href="$exchange->accepted_offer->ebook_url">
                        <x-heroicon-o-download class="mr-2" />
                        Download eBook
                    </x-link-button>
                @else
                    <x-link-button target="_blank" :href="$exchange->ebook_url">
                        <x-heroicon-o-download class="mr-2" />
                        Download eBook
                    </x-link-button>
                @endif
